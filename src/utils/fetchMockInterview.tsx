...
const introductory = `
System: You are an AI interviewer for a {jobRole} position and {skills} given by them.  
Provide a relevant interview question. This is the first question, make it an introductory one.  

Return the response in JSON Object:
{
  "question": "string"
}

Respond ONLY with a JSON object containing a single "question" key.  
Do not include any explanations, markdown formatting, or code blocks.  
The response should be a valid JSON object that can be directly parsed.
...
System: You are an AI interviewer for a {jobRole} position.  
The current question was "{question}" and the candidate's answer was "{answer}".  

Analyze the answer, provide feedback, and generate the next question if appropriate.  

Return the response ONLY as a valid JSON object with no additional text, formatting, or code blocks.  

Response format:
{
  "feedback": "string",
  "nextQuestion": "string or null if the interview should end",
  "analysis": {
...
export default async function fetchMockInterview(
  userResponse: string = "",
  ...
  try {
    ...
    if (nounce === 0) {
      systemInstruction = introductory.replace("{jobRole}", userRole).replace("{skills}", userSkills);

      prompt = `Given job role: ${userRole}\n\nGiven skills: ${userSkills}`;
    } else if (nounce > 0 && nounce <= 4) {
      systemInstruction = feedBackandNext.replace("{jobRole}", userRole)
        .replace("{question}", geminiResponse)
        .replace("{answer}", userResponse);

      prompt = `Job Role: ${userRole}\n\nSkills: ${userSkills}\n\nThe current question was "${geminiResponse}" and the candidate's answer was "${userResponse}"`;
    }
