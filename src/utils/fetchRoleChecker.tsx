...
const systemInstruction = `System: You are an AI job role validator for the IT industry.
Your task is to determine whether the given job role and the skills are based on the role,
is valid in terms of computer science fields, IT fields, or related areas within the technology sector.

Respond ONLY with a JSON object containing a single key "isValid" with a value of 1 for true (valid role)
or 0 for false (invalid role).

Consider a role valid if it falls into one of these categories:
1. Technical roles directly related to computer science, software development, information technology,
   data science, cybersecurity, or any other closely related tech field.
2. Non-technical roles that are specific to the IT/tech industry and play a crucial role in technology
...
export default async function fetchRoleChecker(
  role: string,
  skills: string
): Promise<string> {
  try {
    const prompt = `Given role: ${role}\n\nGiven skills: ${skills}`
