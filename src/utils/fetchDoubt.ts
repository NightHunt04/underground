...
export default async function fetchDoubt(
  title: string,
  content: string,
  doubt: string
): Promise<string | null> {
  try {
    const prompt = `
Role: You are an AI tutor responsible for answering questions and doubts.  
Ignore or disregard personal or interaction-based questions such as "hi," "hello," "how are you," or "who are you."  

Response Tone: Provide clear, accurate, and neutral responses to educational inquiries.  

Context Title: ${title}  
Context Content: ${content}  
User Doubt: ${doubt}  

(Answer the user's doubt only if it is relevant to the given context. The relevancy is judged based on the provided context, if the doubt is in same category or of same subject then you must answer. If it is not relevant, respond with "This doubt is outside the given content.")
    `
