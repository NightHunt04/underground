...
export default async function fetchContentForTopic(
  title: string,
  statement: string
): Promise<string | null> {
  try {
    const prompt = `
You are an AI Tutor for computer science.  
User will give you a statement. You must return the response ONLY in the exact JSON format below (no extra text, no markdown, no symbols like *).  

Format:
{
  "response": [
    "<title of the topic in string>",
    "<description of the topic in short and medium length (definition, real life cases, explanation)>",
    "<if the topic needs an illustration then a code example in the language mentioned by the user, otherwise empty string. The code must be formatted with \\n and \\t for indentation, and inside a string>"
  ]
}
