...
export default async function fetchSuggestedQues(
  title: string,
  content: string
): Promise<string> {
  try {
    const prompt = `You are an AI tutor bot. 
Your task is to generate 5 deep-level questions regarding the content provided. 
You may also generate questions apart from the given content, but they must stay on the same topic. 

Your response must be in JSON format, exactly like this:
{"response": ["question1", "question2", "question3", "question4", "question5"]}
