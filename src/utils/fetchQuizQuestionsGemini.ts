...
export default async function fetchQuizQuestionsGemini(
  title: string,
  statement: string,
  practical: boolean,
  lang: string
  ...
  try {
    ...
    if (!practical) {
      prompt = `You are a bot who doesn't talk but only makes 5 questions and unique and distinct options for those questions along with an answer. 
The response MUST be in exactly this JSON format: 
{"response": [
  { "question": "<question1>", "options": ["option1", "option2", "option3", "option4"], "correct": "correct_answer_from_option" },
  { "question": "<question2>", "options": ["option1", "option2", "option3", "option4"], "correct": "correct_answer_from_option" },
  { "question": "<question3>", "options": ["option1", "option2", "option3", "option4"], "correct": "correct_answer_from_option" },
  { "question": "<question4>", "options": ["option1", "option2", "option3", "option4"], "correct": "correct_answer_from_option" },
  { "question": "<question5>", "options": ["option1", "option2", "option3", "option4"], "correct": "correct_answer_from_option" }
]}
Topic: ${title}
Statement: ${statement}
Language: ${lang}`
    } else {
      prompt = `You are a bot who doesn't talk, you only provide 3 coding/practical questions along with code answers. 
The response MUST be in exactly this JSON format: 
{"response": [
  { "question": "<question1>", "code": "'''${lang}\\n<code1>'''" },
  { "question": "<question2>", "code": "'''${lang}\\n<code2>'''" },
  { "question": "<question3>", "code": "'''${lang}\\n<code3>'''" }
]}
- Each question should describe the task with explanation and 1–2 sample test cases (e.g. Input/Output).
- Code should be valid ${lang}, use \t and \n for formatting.
- Use single quotes inside string literals in code.
Topic: ${title}
Statement: ${statement}
Language: ${lang}`
    }
    ...
    return result // safe JSON string now
  } catch (err) {
