...
export default function SelectedSyllabusTopic({ selectSubjectId, selectChapterId, setSelectChapterId }: Props) {
    ...
    const handleGenerateMCQ = async (isBrief: boolean): Promise<void> => {
        ...
        const chapter = context?.chapters?.find(c => c._id === selectChapterId)
        ...
        if (selectedTopics.length === 0) {
            // Select all topics if none selected
            topicsToBeCovered = [...chapter.topics]
        } else {
             selectedTopics.forEach(topicInd => {
                 if (chapter.topics[topicInd])
                    topicsToBeCovered.push(chapter.topics[topicInd])
             })
        }

        console.log('selected', topicsToBeCovered)

        if (topicsToBeCovered.length > 0) {
            while (1) {
                let response = await fetchSyllabusQues(chapter.title, topicsToBeCovered, isBrief)
    ...
    const subjectName = context?.subjects?.find(s => s._id === selectSubjectId)?.name || 'Unknown Subject'
    const chapterTitle = context?.chapters?.find(c => c._id === selectChapterId)?.title || 'Unknown Chapter'
    const chapterTopics = context?.chapters?.find(c => c._id === selectChapterId)?.topics || []

  return (
    <div className='relative mt-5 flex flex-col items-start justify-start w-[80%] bg-white drop-shadow-lg rounded-lg p-5 gap-2'>
        <button onClick={() => { setSelectChapterId('') }} className="px-3 py-2 text-xs md:text-sm bg-red-500 text-white font-semibold rounded-md absolute right-3 top-3 drop-shadow-md hover:opacity-80 transition-all">Cancel</button>

        <p className='font-semibold'>Selected topic</p>
        <p className='font-sm text-gray-700'>Subject name: <span className='p-1 bg-green-100 font-semibold rounded-md'>{subjectName}</span></p>
        ...
        <p className='mt-10 font-semibold text-xl'>Title: {chapterTitle}</p>
        ...
        <div className='flex flex-col ietms-start justify-start p-2'>
            {chapterTopics.map((item, index: number) => {
                return (
...
// export default function SelectedSyllabusTopic({ selectSubjectId, selectSubjectTopicId, setSelectSubjectTopicId }: Props) {
//     const context = useAppContext()
//     const [selectedTopics, setSelectedTopics] = useState<number[]>([])
//     const [loader, setLoader] = useState<boolean>(false)
//     const [mcqData, setMcqData] = useState<MCQResponse | undefined | string>()
...
//         if (selectedTopics.length === 0) {
//             const subject = context?.subjects?.find(subject => subject._id === selectSubjectId)
//             const topic = subject?.syllabus?.find(topic => topic._id === selectSubjectTopicId)
//             if (topic?.content) {
//                 topic.content.map(topic => topicsToBeCovered.push(topic))
//             }
//         }
