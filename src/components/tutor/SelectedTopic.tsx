...
interface Props {
    ...
    selectChapterTopicId: number // This is the index of the topic within the chapter.topics array
    setSelectChapterTopicId: React.Dispatch<React.SetStateAction<number>>
}
...
export default function SelectedTopic({ 
    selectSubjectId, 
    selectChapterId, 
    selectChapterTopicId, 
    setSelectChapterTopicId 
}: Props) {
    ...
    useEffect(() => {
        const subjectName = context?.subjects?.find((item) => item._id === selectSubjectId)?.name
        const chapter = context?.chapters?.find((item) => item._id === selectChapterId)
        ...
        // Reset topic index when starting new selection
        setTopicIndex(selectChapterTopicId)
    }, [selectSubjectId, selectChapterId, selectChapterTopicId, context?.subjects, context?.chapters])
    ...
    const generateContentForTopic = useCallback(async (): Promise<void> => {
        ...
        const title = chapter.title
        // Access specific topic from the array
        const statement = chapter.topics[topicIndex] 
        ...
        if (!statement) {
            ...
            while (1) {
                ...
                try {
                    ...
                    setTopicIndex(prev => prev + 1) // Move to next topic in the array
                    setLoader(false)
    ...
    }, [topicIndex, selectChapterId, context?.chapters])
    ...
    const handleScrapURL = async (): Promise<void> => {
        ...
        const chapter = context?.chapters?.find((item) => item._id === selectChapterId)
        const title = chapter?.title || ''
        // Use the current topic index (decrement by 1 because it increments after generation)
        const activeTopicIndex = topicIndex > 0 ? topicIndex - 1 : 0
        const statement = chapter?.topics[activeTopicIndex] || ''
  ...
  return (
    <div className='relative mt-5 flex flex-col items-start justify-start w-[80%] bg-white drop-shadow-lg rounded-lg p-5 gap-2'>
        <button onClick={() => { setSelectChapterTopicId(-1) }} className="px-3 py-2 text-xs md:text-sm bg-red-500 text-white font-semibold rounded-md absolute right-3 top-3 drop-shadow-md hover:opacity-80 transition-all">Cancel</button>

        <p className='font-semibold'>Selected topic</p>
        <p className='font-sm text-gray-700'>Subject name: <span className='p-1 bg-green-100 font-semibold rounded-md'>{subject}</span></p>
        ...
        <p className='mt-10 font-semibold text-xl'>Title: {title}</p>
        <p>Topic to be covered</p>

        {/* topics of the selected chapter */}
        <div className='flex flex-col ietms-start justify-start p-2'>
            {context?.chapters?.find((item) => item._id === selectChapterId)?.topics.map((item, index: number) => {
                return (
                    ...
                    return (
                        <div key={index} className='relative flex flex-col items-start justify-start mt-8 mb-20 w-full'>
                            ...
                            {content[2].length !== 0 && <div className='mt-5 w-full'>
                                    <SyntaxHighlighter className="rounded-md" style={vscDarkPlus}>
                                        { content[2] }
                                    </SyntaxHighlighter>
                                </div>}
                        </div>
...
//         const chapter = context?.chapters?.find((item) => item._id === selectChapterId)
//         const title = chapter?.title || ''
//         // const statement = chapter?.topics[0] || ''

        
//         setTitle(title)
//         // setStatement(statement)
//         setSubject(subjectName || '')
//     }, [selectSubjectId, selectChapterTopicId])
...
//         const chapter = context?.chapters?.find((item) => item.subject._id === selectSubjectId && item.indexNumber === selectChapterTopicId)
//         const title = chapter?.title || ''
//         const statement = chapter?.topics[topicIndex] || ''
