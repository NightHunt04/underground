...
export default function AssignmentList({ assignments, setSelectSubjectAssignmentIndex, setSelectTempSubjectId }: Props) {
  return (
    <div className="w-full rounded-md drop-shadow-lg mt-2 p-2 border-2 border-gray-300 flex flex-col items-center justify-center">
        ...
        {assignments.length === 0 ? <p className="text-gray-500 mb-5">No assignments found for this subject.</p> : 
        assignments.map((assignment, index: number) => {
            return (
                <div key={assignment._id} onClick={() => {
                    ...
                    <div className="flex flex-col items-start justify-start">
                        ...
                        <p className="text-xs md:text-sm text-gray-700">{assignment.description.substring(0, 100)}...</p>
                    </div>
                </div>
            )
        })}
