...
interface Props {
    ...
    setSelectSubjectId: React.Dispatch<React.SetStateAction<string>>

    selectTempSubjectId: string,
    setSelectTempSubjectId: React.Dispatch<React.SetStateAction<string>>
...
export default function QuickQuiz ({ selectSubjectAssignmentIndex, selectSubjectId, selectTempSubjectId, setLoader, loader, quizProblems, setQuizProblems, setSelectSubjectAssignmentIndex, setSelectSubjectId, setSelectTempSubjectId, loader1, setLoader1, codeProblems, setCodeProblems }: Props) {
    ...
    const handleGenerateQuiz = async (practical: boolean) => {
        ...
        // Get filtered assignments for the current subject
        const subjectAssignments = context?.assignments?.filter(a => a.subject._id === selectSubjectId) || []
        const currentAssignment = subjectAssignments[selectSubjectAssignmentIndex]

        const title = currentAssignment?.title
        const statement = currentAssignment?.description // using description as statement
        
        let lang = currentAssignment?.language && currentAssignment.language !== 'any' ? currentAssignment.language : selectedLang 
        if (selectedLang === 'Select Language' && (!currentAssignment?.language || currentAssignment.language === 'any')) {
                ...
                {context?.subjects?.map((sub, index: number) => {
                    ...
                    return (
                        <div key={sub._id} className="flex flex-col items-start justify-start w-full">
                            ...
                            {selectTempSubjectId === sub._id && 
                                <AssignmentList 
                                    assignments={filteredAssignments} 
                                    setSelectSubjectAssignmentIndex={setSelectSubjectAssignmentIndex} 
                                    setSelectTempSubjectId={setSelectTempSubjectId}/>}
                        </div>
                    )
                })}

                {/*  once the assignment is selected */}
                {selectSubjectAssignmentIndex !== -1 && currentAssignment &&
                    <div className="relative px-4 py-3 mt-16 border-2 border-green-500 rounded-lg flex flex-col items-start justify-start w-full">
                        ...
                        <p className="mt-2 font-semibold text-lg md:text-xl">Title: {currentAssignment.title}</p>
                        <p className="text-gray-700 font-medium">Statement: {currentAssignment.description}</p>

                        <div className={`flex ${!currentAssignment.language || currentAssignment.language === 'any' ? 'flex-col gap-1' : 'flex-row gap-5'} mt-10 items-start justify-center`}>
                            ...
                            {(!currentAssignment.language || currentAssignment.language === 'any') &&
